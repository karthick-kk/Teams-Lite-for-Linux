...
void notifications_init() {
    if (!notify_init("Teams Lite for Linux")) {
        fprintf(stderr, "[tfl] Failed to initialize notifications\n");
        return;
    }
...
void notifications_show(const std::string& title, const std::string& body) {
    ...
    if (!n) {
        fprintf(stderr, "[tfl] Failed to create notification\n");
        return;
    }
    ...
    g_signal_connect_swapped(n, "closed", G_CALLBACK(g_object_unref), n);
