#!/bin/bash
# Build CEF from source with H.264 encoding via OpenH264 (open-source)
# Requirements: ~50GB disk, ~16GB RAM, 2-4 hours on 32 cores
set -euo pipefail
...
echo "=== CEF Source Build (OpenH264 H.264 encoding) ==="
...
echo "  ffmpeg_branding=Chromium (open-source FFmpeg only)"
...
# ffmpeg_branding=Chromium: use open-source FFmpeg (no proprietary AAC/etc)
# rtc_use_h264=true: use Cisco's OpenH264 (BSD-licensed) for WebRTC H.264 encoding
export GN_DEFINES="is_official_build=true proprietary_codecs=true ffmpeg_branding=Chromium rtc_use_h264=true use_sysroot=false symbol_level=1 is_cfi=false use_thin_lto=false"
export CEF_USE_GN=1
...
# Symlink resources
ln -sf /tmp/cef/Resources/icudtl.dat /tmp/cef/Release/icudtl.dat
ln -sf /tmp/cef/Resources/chrome_100_percent.pak /tmp/cef/Release/chrome_100_percent.pak
ln -sf /tmp/cef/Resources/chrome_200_percent.pak /tmp/cef/Release/chrome_200_percent.pak
ln -sf /tmp/cef/Resources/resources.pak /tmp/cef/Release/resources.pak
ln -sf /tmp/cef/Resources/locales /tmp/cef/Release/locales
